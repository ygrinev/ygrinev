...
using FCT.EPS.WSP.SCPRTPTA.Implementation.Properties;
using Microsoft.Practices.EnterpriseLibrary.Common.Configuration;

namespace FCT.EPS.WSP.SCPRTPTA.Implementation
{
    public class CreditPaymentNotificationAgent : MarshalByRefObject, IAgent, IDisposable
    {
        private Timer timer;
        ...
        //NonGenericDirectAccess _nonGenericDirectAccess;
        PaymentTrackingWebServiceClient _paymentTrackingServiceClient = null;
        private bool _disposed = false;

        public CreditPaymentNotificationAgent()
        {
            ...
            catch (Exception ex)
            {
                ...
                LoggingHelper.LogErrorActivity("CreditPaymentNotificationAgent : Exception was thrown.", ex);
            }
        ...
        private void TimerTick(object stateInfo)
        {
            ...
            try
            {
                ...
                { 
                    ...
                    if (_paymentTrackingServiceClient == null) throw new CreditNotificationAgentException("Unable to create Payment Tracking WebService Client");
                }

                CreditNotificationRequestHandler service = new CreditNotificationRequestHandler(_paymentTrackingServiceClient);

                if (service != null)
                {
                    service.SubmitCreditPaymentRequestToPaymentTracker(this.recordsToGet, this.maxAllowedRetry);
                }
