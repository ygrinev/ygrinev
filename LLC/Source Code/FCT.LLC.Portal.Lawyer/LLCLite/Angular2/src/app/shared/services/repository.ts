...
export interface IRepository<T> {
    ...
    whereString(predicatekey: string, predicatevalue: string): Promise<T[]>;
...
export class Repository<T> implements IRepository<T> {
    ...
    whereString(predicatekey: string, predicatevalue: string) : Promise<T[]> {
        var p = new Predicate(predicatekey, "eq", predicatevalue);
        let query = this.baseQuery().where(p);
    ...
    executeQuery(query: EntityQuery, fetchStrategy?: FetchStrategySymbol): Promise<T[]> {
