...
namespace SQLImportExport
{
    ...
    public static class Excel
    {
        ...
        public static void ReadWorkBook(string fileName)
        {
            ...
            List<string> updateStatementList = new List<string>();
            string updatequery = string.Empty;
            StringBuilder updateList = new StringBuilder();
            StringBuilder keyList = new StringBuilder();
            string updateFormat = "{0}='{1}'";
            ...
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Open(fileName, false))
            {
                    ...
                    while (reader.Read())
                    {
                        if (reader.ElementType == typeof(Row))
                        {
                               ...
                               // Console.WriteLine("HEADER");
                                ...
                                do
                                {
                                    if (reader.ElementType == typeof(Cell))
                                    {
                                        ...
                                        if (colNum == Program.KEY_1_COLINDEX)
                                        {
                                            KEY_1_COLNAME = cellValue;
                                        }
                                        else if (colNum == Program.KEY_2_COLINDEX)
                                        {
                                            KEY_2_COLNAME = cellValue;
                                        }
                                        else if (colNum == Program.KEY_3_COLINDEX)
                                        {
                                            KEY_3_COLNAME = cellValue;
                                        }
                                        else if (colNum == Program.KEY_4_COLINDEX)
                                        {
                                            KEY_4_COLNAME = cellValue;
                                        }
                                        else if (colNum == Program.UPDATE_1_COLINDEX)
                                        {
                                            UPDATE_1_COLNAME = cellValue;
                                        }
                                        else if (colNum == Program.UPDATE_2_COLINDEX)
                                        {
                                            UPDATE_2_COLNAME = cellValue;
                                        }
                                        else if (colNum == Program.UPDATE_3_COLINDEX)
                                        {
                                            UPDATE_3_COLNAME = cellValue;
                                        }
                                        else if (colNum == Program.UPDATE_4_COLINDEX)
                                        {
                                            UPDATE_4_COLNAME = cellValue;
                                        }
                            ...
                            else
                            {
                                ...
                                do
                                {
                                    if (reader.ElementType == typeof(Cell))
                                    {
                                        ...
                                        if (c != null)
                                        {
                                            ...
                                            if (colNum == Program.KEY_1_COLINDEX)
                                            {
                                                KEY_1_COLVALUE = cellValue;
                                                keyList.AppendFormat(keyFormat, KEY_1_COLNAME, KEY_1_COLVALUE);
                                            }
                                            else if (colNum == Program.KEY_2_COLINDEX)
                                            {
                                                KEY_2_COLVALUE = cellValue;
                                                keyList.Append(" AND ");
                                                keyList.AppendFormat(keyFormat, KEY_2_COLNAME, KEY_2_COLVALUE);
                                            }
                                            else if (colNum == Program.KEY_3_COLINDEX)
                                            {
                                                KEY_3_COLVALUE = cellValue;
                                                keyList.Append(" AND ");
                                                keyList.AppendFormat(keyFormat, KEY_3_COLNAME, KEY_3_COLVALUE);
                                            }
                                            else if (colNum == Program.KEY_4_COLINDEX)
                                            {
                                                KEY_4_COLVALUE = cellValue;
                                                keyList.Append(" AND ");
                                                keyList.AppendFormat(keyFormat, KEY_4_COLNAME, KEY_4_COLVALUE);
                                            }
                                            else if (colNum == Program.UPDATE_1_COLINDEX)
                                            {
                                                UPDATE_1_COLVALUE = cellValue.Replace("'", "''");
                                                updateList.AppendFormat(updateFormat, UPDATE_1_COLNAME, UPDATE_1_COLVALUE);
                                            }
                                            else if (colNum == Program.UPDATE_2_COLINDEX)
                                            {
                                                UPDATE_2_COLVALUE = cellValue.Replace("'", "''");
                                                updateList.Append(",");
                                                updateList.AppendFormat(updateFormat, UPDATE_2_COLNAME, UPDATE_2_COLVALUE);

                                            }
                                            else if (colNum == Program.UPDATE_3_COLINDEX)
                                            {
                                                UPDATE_3_COLVALUE = cellValue.Replace("'", "''");
                                                updateList.Append(",");
                                                updateList.AppendFormat(updateFormat, UPDATE_3_COLNAME, UPDATE_3_COLVALUE);
                                            }
                                            else if (colNum == Program.UPDATE_4_COLINDEX)
                                            {
                                                UPDATE_4_COLVALUE = cellValue.Replace("'", "''");
                                                updateList.Append(",");
                                                updateList.AppendFormat(updateFormat, UPDATE_4_COLNAME, UPDATE_4_COLVALUE);
                                            }
                                ...
                                updatequery = string.Format(Program.UPDATEQUERY_FORMAT, Program.TABLENAME, updateList.ToString(),
                                           keyList.ToString());
                                updateStatementList.Add(updatequery);
                                updateList.Clear();
                                keyList.Clear();
                ...
                GenerateSQLScript(updateStatementList);
            }
        }

        public static string KEY_1_COLNAME = string.Empty;
        public static string KEY_2_COLNAME = string.Empty;
        public static string KEY_3_COLNAME = string.Empty;
        public static string KEY_4_COLNAME = string.Empty;
        
        public static string UPDATE_1_COLNAME = string.Empty;
        public static string UPDATE_2_COLNAME = string.Empty;
        public static string UPDATE_3_COLNAME = string.Empty;
        public static string UPDATE_4_COLNAME = string.Empty;

        public static string KEY_1_COLVALUE = string.Empty;
        public static string KEY_2_COLVALUE = string.Empty;
        public static string KEY_3_COLVALUE = string.Empty;
        public static string KEY_4_COLVALUE = string.Empty;

        public static string UPDATE_1_COLVALUE = string.Empty;
        public static string UPDATE_2_COLVALUE = string.Empty;
        public static string UPDATE_3_COLVALUE = string.Empty;
        public static string UPDATE_4_COLVALUE = string.Empty;

        //public static string UPDATEQUERY_FORMAT = "UPDATE {0} SET {1}={2} WHERE {3}={4}";
        //public static void ReadWorkBook_org(string fileName)
        //{        
        //    List<string> updateStatementList = new List<string>();
        //    string updatequery = string.Empty;
        //    StringBuilder updateList = new StringBuilder();
        ...
        //                updatequery = string.Format(Program.UPDATEQUERY_FORMAT, Program.TABLENAME, updateList.ToString(), 
        //                                                            string.Format(keyFormat, KEY_1_COLNAME, KEY_1_COLVALUE));
        //                updateStatementList.Add(updatequery);
        //                updateList.Clear();
        //            }
        //            rowNum++;
        ...
        public static void GenerateSQLScript(List<string> updateStatementList)
        {
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(Program.FILENAME + ".sql"))
            {
                foreach (string line in updateStatementList)
                {
        ...
        /// <returns>True if succesful</returns>
        public static bool SetCellValue(DocumentFormat.OpenXml.Packaging.SpreadsheetDocument spreadsheet,
            DocumentFormat.OpenXml.Spreadsheet.Worksheet worksheet,
            uint columnIndex,
            uint rowIndex,
            string stringValue, bool useSharedString, bool save = true)
        {
